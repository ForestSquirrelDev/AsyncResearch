...
#### ❔Вопрос 3: Сколько места на хипе занимает стейт машина?

📖 Ответ: Без дополнительно захваченного контекста - 24 B.

#### ❔Вопрос 4: Что будет если объект, создавший `async Task` стейт машину и ждущий `Continuation`, станет Eligible For GC (на него ни у кого не осталось ссылок)?

📖 Ответ: Зависит от того, что происходит внутри стейт машины. Если там происходит что-то, по механизму похожее на `Task.Delay()` - объект будет жить до тех пор, 
...
#### ❔Вопрос 5: В каких случаях стактрейс исключения в асинхронном методе может потеряться?
...
#### ❔Вопрос 6: Когда мы говорим об аллокациях на Стейт машину, о каких размерах вообще идёт речь?

📖Ответ: Сама по себе стейт машина, без захвата контекста, весит немного: 24 байта. Но чем больше у неё обвязочной инфраструктуры и захваченных переменных, тем она дороже:
метод с `await Task.Yield()` будет весить 96 байт. 

#### ❔Вопрос 7: Как исключение в асинхронном методе пробрасывается наверх по стеку вызовов?
...
#### ❔Вопрос 8: Что на самом деле происходит под капотом, когда мы говорим "асинхронный код выполнится синхронно"?
...
#### ❔Вопрос 9: Стейт машина создастся, даже если код исполнится синхронно?

📖Ответ: Да. Компилятор создаст стейт машину на любой `async` метод.

#### ❔Вопрос 10: Если стейт машина исполнится синхронно, она всё равно аллоцируется в куче, или нет?

📖Ответ: Нет. Стейт машина сразу вернёт управление вызывающему методу/стейт машине, без аллокаций в управляемой куче.

#### ❔Вопрос 11: Чем отличается `async Void` от `async Task`?
