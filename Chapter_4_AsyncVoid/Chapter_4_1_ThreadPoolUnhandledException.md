...
В упрощённом виде, мы прикинулись программой, которая крутит какой-то цикл в управляющем потоке. И через секунду работы данного цикла, в `async void` методе выбрасывается
исключение.
