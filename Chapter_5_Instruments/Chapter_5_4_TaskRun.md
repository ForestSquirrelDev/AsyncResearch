...
````
Если у нас нет контекста синхронизации, после `await Task.Yield();` мы окажемся в `ThreadPool`, и выполним работу там. Но если контекст синхронизации есть, после `Task.Yield()`
